...
export default function Inicio() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/api/actividades")
  ...
  const resumenPorUsuario = usuarios.map((usuario) => {
    const actividadesUsuario = actividades.filter(
      (a) =>
        a.id_usuario_asignado === usuario.id_usuario &&
        estaSemana(a.fecha_programada)
    );
    const total = actividadesUsuario.length;
    const completadas = actividadesUsuario.filter(
  ...
  return (
    <div className="p-6 space-y-10">
      ...
      <div id="card-actividades" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <div id="card-urgentes" className="bg-white p-4 rounded shadow">
        <h3 className="text-lg font-semibold mb-3">
          🔥 Actividades Urgentes (Alta prioridad)
        </h3>
        {actividadesUrgentes.length === 0 ? (
          <p className="text-sm text-gray-500">
            No hay actividades urgentes pendientes.
          </p>
        ) : (
          actividadesUrgentes.map((a) => (
            <div key={a.id_actividad} className="border-b pb-2 mb-2">
              ...
              <p className="text-sm text-gray-600">
                {a.nombre_empresa} — {a.nombre_usuario}
              </p>
              <p className="text-sm text-gray-500">
                Fecha programada: {formatFecha(a.fecha_programada)}
              </p>
            </div>
          ))
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div id="card-cotizaciones" className="bg-white p-4 rounded shadow">
          <h3 className="text-lg font-semibold mb-3">
          ...
          {cotizaciones.map((c) => (
            <div key={c.id_cotizacion} className="border-b pb-2 mb-2">
              ...
              <p className="text-sm text-gray-500">
                Emisión: {formatFecha(c.fecha_emision)}
              </p>
              <p className="text-sm">
        ...
        <div id="card-documentos" className="bg-white p-4 rounded shadow">
          <h3 className="text-lg font-semibold mb-3">
