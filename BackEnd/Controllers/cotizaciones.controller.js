...
const obtenerCotizacionPorId = (req, res) => {
  ...
  CotizacionModel.obtenerCotizacionPorId(id, (err, cotResult) => {
    ...
    DetalleCotizacionModel.obtenerDetallesPorCotizacionId(id, (err, detalles) => {
      if (err) {
        console.error("Error al obtener detalles:", err.message)
        return res.status(500).json({ error: "Error al obtener detalles de la cotización" })
      }
...
const crearCotizacion = (req, res) => {
  ...
  if (!id_empresa || !fecha_emision || !fecha_vencimiento || !estado || !total || !Array.isArray(detalles)) {
    return res.status(400).json({ error: "Faltan campos obligatorios o detalles inválidos" })
  }

  const nuevaCotizacion = { id_empresa, fecha_emision, fecha_vencimiento, estado, total, enlace_pago }
  ...
  CotizacionModel.crearCotizacion(nuevaCotizacion, (err, result) => {
    ...
    detalles.forEach((detalle, index) => {
      DetalleCotizacionModel.crearDetalleCotizacion(id_cotizacion, detalle, (err) => {
        if (err) errores.push({ index, error: err.message })
      })
    })

    if (errores.length) {
      return res.status(500).json({ mensaje: "Cotización creada con errores en detalles", errores })
    }
...
const eliminarCotizacion = (req, res) => {
  ...
  DetalleCotizacionModel.eliminarDetallesPorCotizacionId(id, (err) => {
    if (err) {
      ...
      return res.status(500).json({ error: "Error al eliminar detalles de la cotización" })
    }
